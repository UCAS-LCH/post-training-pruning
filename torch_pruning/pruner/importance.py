    ...
    def __call__(self, group: Group):
        ...
        for i, (dep, idxs) in enumerate(group):
            ...
            ####################
                ...
                function.prune_multihead_attention_out_channels,
            ]:
                local_norm = 0
                if layer.q_proj_weight is not None:
                    for sub_weight in [layer.q_proj_weight, layer.k_proj_weight, layer.v_proj_weight]:
                        w_out = sub_weight.data[idxs]
                        local_norm += w_out.abs().pow(self.p).sum(1)
                        w_in = sub_weight.data[:, idxs]
                        local_norm += w_in.abs().pow(self.p).sum(0)
                else:
                    for k in range(3):
                        idxs_repeated = [i+ k * layer.embed_dim for i in idxs]
                        w_out = layer.in_proj_weight.data[idxs_repeated]
                        local_norm += w_out.abs().pow(self.p).sum(1)
                        w_in = layer.in_proj_weight.data[:, idxs]
                        local_norm += w_in.abs().pow(self.p).sum(0)
                
                w_out = layer.out_proj.weight.data[idxs]
                local_norm += w_out.abs().pow(self.p).sum(1)
                w_in = layer.out_proj.weight.data[:, idxs]
                local_norm += w_in.abs().pow(self.p).sum(0)
